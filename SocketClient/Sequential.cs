...
namespace Sequential {
    ...
    public class SimpleClient
    {
        ...
        public SimpleClient(int id, Setting settings)
        {
            ...
            // cmd = "ps :open -a preview :open -a whatsapp :ls -all";
        
            cmd = settings.votingList;

            string[] cmds = cmd.Split(settings.commands_sep);

            int choice = random.Next(0, cmds.Length);

            cmd_message = "ClientId="+client_id.ToString()+settings.command_msg_sep+cmds[choice];
